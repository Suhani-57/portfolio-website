/* Unseen case study — page-local behaviour only.
   Deliberately standalone: script.js drives the homepage's project list and
   preview card and would throw here. This file is the same page chrome the
   About, Fluence and Hoopr pages use — mobile menu, copy-email toast and the
   shared .reveal system — so the footer copied from index.html animates in
   exactly as it does there.

   The wireframe and hi-fi strips are pure CSS animation; nothing here drives
   them, so they keep moving even if this script never runs. */
(function(){
  var prefersReduced = window.matchMedia && window.matchMedia('(prefers-reduced-motion: reduce)').matches;

  // ---------- mobile menu ----------
  var menu = document.getElementById('unMobileMenu');
  var toggle = document.getElementById('unNavToggle');
  var close = document.getElementById('unMobileClose');
